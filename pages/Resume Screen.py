...
resume = st.file_uploader("Upload Candidate's resume", type=["pdf"])
...
def initialize_session_state_resume():
    ...
    if "resume_history" not in st.session_state:
        ...
        st.session_state.resume_history.append(Message(origin="ai",
                                                       message="Hello, I am your interviewer today. I will ask you "
                                                               "some questions regarding your resume and your "
                                                               "experience. Please start by saying hello or "
                                                               "introducing yourself. Note: The maximum length of "
    ...
    if "resume_guideline" not in st.session_state:
        ...
        st.session_state.resume_guideline = RetrievalQA.from_chain_type(
            ...
            retriever=st.session_state.retriever, memory=st.session_state.resume_memory).run(
            "Create an interview guideline and prepare only two questions for each topic. Make sure the questions "
            "tests the knowledge")
    ...
    if "resume_screen" not in st.session_state:
        ...
        PROMPT = PromptTemplate(
            ...
            Candidate: {input}
            AI: """)
        st.session_state.resume_screen = ConversationChain(prompt=PROMPT, llm=llm,
...
if position and resume:
    ...
    feedback = st.sidebar.button("Get interview Feedback")
    guideline = st.sidebar.button("Guidelines for Evaluation!", disabled=True)
